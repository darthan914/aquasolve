...
@section('title')
  <title>{{ $name->title }} | Change {{ $Product->name }} | Product</title>
@endsection
...
<div class="row">
  <div class="col-md-12 col-sm-12 col-xs-12">
    <div class="x_panel">
      <div class="x_title">
        <h2>Change {{ $Product->name }} Product<small></small></h2>
      ...
      <div class="x_content">
        <form action="{{ route('backend.product.change.store', ['id'=>$Product->id]) }}" method="POST" class="form-horizontal form-label-left" enctype="multipart/form-data" novalidate>
          {{ csrf_field() }}
          ...
          <div class="item form-group {{ $errors->has('name') ? 'has-error' : ''}}">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              <input id="name" class="form-control col-md-7 col-xs-12" name="name" placeholder="Name" required="required" type="text" value="{{ old('name',$Product->name) }}">
              @if($errors->has('name'))
          ...
          <div class="item form-group {{ $errors->has('website') ? 'has-error' : ''}}">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              <input id="website" class="form-control col-md-7 col-xs-12" name="website" placeholder="Website" required="required" type="text" value="{{ old('website',$Product->website) }}">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              <select id="category" class="form-control col-md-7 col-xs-12" name="category" required="required">
                @foreach($ProdukCategory as $list)
                  <option value="{{$list->id}}" {{ old('category',$Product->category) == $list->id ? 'selected' : '' }}>{{$list->name}}</option>
                @endforeach
              </select>
          ...
          <div class="item form-group {{ $errors->has('title_picture') ? 'has-error' : ''}}">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              ...
              @endif
              @if($Product->title_picture != '')
              <a href="{{ asset('amadeo/images/'.$Product->title_picture) }}" target="_blank">
                <img src="{{ asset('amadeo/images/'.$Product->title_picture) }}" style="height: 120px;">
              </a>
              <label class="inline-checkbox"><input type="checkbox" name="remove_title_picture"> Remove</label>
              @endif
            </div>
          </div>
          ...
          <div class="item form-group {{ $errors->has('picture') ? 'has-error' : ''}}">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              ...
              @endif
              <a href="{{ asset('amadeo/images/'.$Product->picture) }}" target="_blank">
                <img src="{{ asset('amadeo/images/'.$Product->picture) }}" style="height: 120px;">
              </a>
            </div>
          </div>
          ...
          <div class="item form-group {{ $errors->has('background_picture') ? 'has-error' : ''}}">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              ...
              @endif
              <a href="{{ asset('amadeo/images/'.$Product->background_picture) }}" target="_blank">
                <img src="{{ asset('amadeo/images/'.$Product->background_picture) }}" style="height: 120px;">
              </a>
            </div>
          </div>
